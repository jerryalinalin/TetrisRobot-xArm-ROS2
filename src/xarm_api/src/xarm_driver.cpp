...
{   
        ...
        if (add_gripper) {
            ...
            std::thread([this]() {
                ...
                while (ret == 0 && !xarm_gripper_init_loop_)
                {
                    ...
                    _pub_xarm_gripper_joint_states(static_cast<int>(curr_pos));
                }
            }).detach();
        }
    }
    ...
    void XArmDriver::_xarm_gripper_action_execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::GripperCommand>> goal_handle)
    {
        ...
        if (ret != 0) {
            ...
            ret = arm->get_gripper_err_code(&err);
            RCLCPP_WARN(node_->get_logger(), "set_gripper_mode, ret=%d, err=%d, curr_pos=%.0f", ret, err, curr_pos);
            return;
        }
        ...
        if (ret != 0) {
            ...
            ret = arm->get_gripper_err_code(&err);
            RCLCPP_WARN(node_->get_logger(), "set_gripper_enable, ret=%d, err=%d, curr_pos=%.0f", ret, err, curr_pos);
            return;
        }
        ...
        if (ret != 0) {
            ...
            ret = arm->get_gripper_err_code(&err);
            RCLCPP_WARN(node_->get_logger(), "set_gripper_speed, ret=%d, err=%d, curr_pos=%.0f", ret, err, curr_pos);
            return;
        }
        ...
        std::thread([this, &target_pos, &is_move, &curr_pos]() {
            ...
            RCLCPP_INFO(node_->get_logger(), "set_gripper_position, ret=%d, err=%d, curr_pos=%.0f", ret2, err, curr_pos);
            is_move = false;
        }).detach();
        int cnt = 0;
        ...
        while (is_move && rclcpp::ok())
        {
            ...
            //     xarm_gripper_result_->position = _xarm_gripper_pos_convert(curr_pos);
            //     goal_handle->canceled(xarm_gripper_result_);
            //     RCLCPP_INFO(this->get_logger(), "Goal canceled, curr_pos=%.0f", curr_pos);
            //     return;
            // }
        }
        arm->get_gripper_position(&curr_pos);
        RCLCPP_INFO(node_->get_logger(), "move finish, curr_pos=%.0f", curr_pos);
    ...
    void XArmDriver::_init_bio_gripper(void)
    {
        ...
        if (add_bio_gripper) {
            ...
            std::thread([this]() {
                ...
                {
                    std::this_thread::sleep_for(std::chrono::milliseconds(500));
                    _pub_bio_gripper_joint_states(static_cast<int>(curr_pos));
                }
            }).detach();
        }
    }
    ...
    void XArmDriver::_bio_gripper_action_execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::GripperCommand>> goal_handle)
    {
        ...
        //     }
        //     ret = arm->get_bio_gripper_error(&err);
        //     RCLCPP_WARN(node_->get_logger(), "set_bio_gripper_enable, ret=%d, err=%d, curr_pos=%.0f", ret, err, curr_pos);
        //     return;
        // }
        ...
        //     }
        //     ret = arm->get_bio_gripper_error(&err);
        //     RCLCPP_WARN(node_->get_logger(), "set_bio_gripper_speed, ret=%d, err=%d, curr_pos=%.0f", ret, err, curr_pos);
        //     return;
        // }
        ...
        std::thread([this, &target_pos, &is_move, &curr_pos]() {
            ...
            RCLCPP_INFO(node_->get_logger(), "set_bio_gripper_position, ret=%d, err=%d, curr_pos=%.0f", ret2, err, curr_pos);
            is_move = false;
        }).detach();
        int cnt = 0;
        bool is_succeed = false;
        ...
        RCLCPP_INFO(node_->get_logger(), "bio move finish, curr_pos=%.0f", curr_pos);
        if (rclcpp::ok() && !is_succeed) {
            bio_gripper_result_->position = _bio_gripper_pos_convert(curr_pos);
            try {
                goal_handle->succeed(bio_gripper_result_);
